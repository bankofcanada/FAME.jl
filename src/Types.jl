...
@inline throw_wrong_value(kind::AbstractString, val) = throw(ArgumentError("Invalid $kind $val"))

@inline check_value(val, valid::_VT) = throw_wrong_value(Val(valid), val)
@inline check_value(val::AbstractString, valid::_VT) = check_value(Symbol(lowercase(val)), valid)
@inline check_value(val::Symbol, valid::_VT) = haskey(valid, val) ? val : throw_wrong_value(Val(valid), val)
@inline check_value(val::Integer, valid::_VT) = val ∈ valid ? val : throw_wrong_value(Val(valid), val)
...
Holds the internal state of FAME.jl as a global variable. There is no need for
end users to play with this.
"""
